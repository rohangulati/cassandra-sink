...
class CassandraTableWriter[T] private (connector: CassandraConnector,
  ...
  private def deleteQueryTemplate(deleteColumns: ColumnSelector): String = {
    ...
    val whereClause =
      quotedColumnNames(primaryKey).map(c => s"$c = :$c").mkString(" AND ")

    val usingTimestampClause =
      if (timestampSpec.nonEmpty) s"USING ${timestampSpec.get}" else ""

    if (ttlEnabled)
      logWarning(s"${writeConf.ttl} is ignored for DELETE query")

    s"DELETE ${deleteColumnsClause} FROM ${quote(keyspaceName)}.${quote(
      tableName)} $usingTimestampClause WHERE $whereClause"
  }

  private[connector] lazy val queryTemplateUsingUpdate: String = {
    val (primaryKey, regularColumns) = columns.partition(_.isPrimaryKeyColumn)
    val (counterColumns, nonCounterColumns) =
      regularColumns.partition(_.isCounterColumn)
    ...
    val setNonCounterColumnsClause = for {
      ...
      collectionBehavior match {
        case Some(CollectionAppend) =>
          s"$quotedName = $quotedName + :$quotedName"
        case Some(CollectionPrepend) =>
          s"$quotedName = :$quotedName + $quotedName"
        case Some(CollectionRemove) =>
          s"$quotedName = $quotedName - :$quotedName"
    ...
    val setClause =
      (setNonCounterColumnsClause ++ setCounterColumnsClause).mkString(", ")
    val whereClause =
      quotedColumnNames(primaryKey).map(c => s"$c = :$c").mkString(" AND ")
  ...
  private lazy val timestampSpec: Option[String] = {
    writeConf.timestamp match {
      case TimestampOption(PerRowWriteOptionValue(placeholder)) =>
        Some(s"TIMESTAMP :$placeholder")
      case TimestampOption(StaticWriteOptionValue(value)) =>
    ...
    if (columns.filter(_.isCounterColumn).nonEmpty) {
      ...
      columnSelector.forall {
        ...
        case cn: CollectionColumnName =>
          ...
          val isNotList =
            !tableDef.columnByName(name).columnType.isInstanceOf[ListType[_]]
          behavior match {
  ...
  private def prepareStatement(queryTemplate: String,
                               session: Session): PreparedStatement = {
    try {
  ...
  def batchRoutingKey(
      session: Session,
      routingKeyGenerator: RoutingKeyGenerator)(bs: BoundStatement): Any = {
    writeConf.batchGroupingKey match {
      case BatchGroupingKey.None => 0

      case BatchGroupingKey.ReplicaSet =>
        if (bs.getRoutingKey(ProtocolVersion.NEWEST_SUPPORTED,
                             CodecRegistry.DEFAULT_INSTANCE) == null)
          bs.setRoutingKey(routingKeyGenerator(bs))
        session.getCluster.getMetadata
          .getReplicas(keyspaceName,
                       bs.getRoutingKey(ProtocolVersion.NEWEST_SUPPORTED,
                                        CodecRegistry.DEFAULT_INSTANCE))
          .hashCode() // hash code is enough
  ...
  /**
    ...
    */
  def update(data: Iterator[T]): Unit =
    writeInternal(queryTemplateUsingUpdate, data)

  /**
    * Write data with Cql INSERT statement
    */
  def insert(data: Iterator[T]): Unit =
    writeInternal(queryTemplateUsingInsert, data)
  ...
  private def writeInternal(queryTemplate: String, data: Iterator[T]): Unit = {
    connector.withSessionDo { session =>
      ...
      val boundStmtBuilder =
        new BoundStatementBuilder(rowWriter,
                                  stmt,
                                  protocolVersion = protocolVersion,
                                  ignoreNulls = writeConf.ignoreNulls)

      val batchStmtBuilder =
        new BatchStatementBuilder(batchType,
                                  routingKeyGenerator,
                                  writeConf.consistencyLevel)
      val batchKeyGenerator = batchRoutingKey(session, routingKeyGenerator) _
      val batchBuilder =
        new GroupingBatchBuilder(boundStmtBuilder,
                                 batchStmtBuilder,
                                 batchKeyGenerator,
                                 writeConf.batchSize,
                                 ...
                                 rowIterator)
      val rateLimiter =
        new RateLimiter((writeConf.throughputMiBPS * 1024 * 1024).toLong,
                        1024 * 1024)

      logDebug(
        s"Writing data partition to $keyspaceName.$tableName in batches of ${writeConf.batchSize}.")
...
object CassandraTableWriter {
  ...
  private def checkMissingPrimaryKeyColumns(table: TableDef,
  ...
  private def checkMissingPartitionKeyColumns(table: TableDef,
                                              columnNames: Seq[String]) {
  ...
  private def onlyPartitionKeyAndStatic(table: TableDef,
                                        columnNames: Seq[String]): Boolean = {
    val nonPartitionKeyColumnNames = columnNames.toSet -- table.partitionKey
      .map(_.columnName)
    val nonPartitionKeyColumnRefs = table.allColumns
      .filter(columnDef =>
        nonPartitionKeyColumnNames.contains(columnDef.columnName))
    nonPartitionKeyColumnRefs.forall(columnDef =>
      columnDef.columnRole == StaticColumn)
  }
  ...
  /**
    ...
    */
  private def checkCollectionBehaviors(table: TableDef,
    ...
    val collectionBehaviorColumnNames =
      refsWithCollectionBehavior.map(_.columnName)
